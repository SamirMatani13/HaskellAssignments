...
addSNat (Positive x) (Positive y) =
  case (x,y) of
    (Succ n,m) -> natToSign (Positive 'a') (Succ(signToNat(addSNat (Positive n) (Positive m))))
    (Zero,m) -> Positive m
...
addSNat (Negative x) (Negative y) =
  case (x,y) of
    (Succ n,m) -> natToSign (Negative 'a') (Succ(signToNat(addSNat (Negative n) (Negative m))))
    (Zero,m)   -> Negative m

natToSign :: Signed Char -> Nat -> Signed Nat
natToSign (Negative 'a') m = Negative m
natToSign (Positive 'a') m = Positive m

signToNat :: Signed Nat -> Nat
signToNat (Positive n) = n
signToNat (Negative n) = n
