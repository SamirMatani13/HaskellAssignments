...
mapWithIndex f xs = map (f) (zip [0.. length xs] xs)
...
toListType :: [a] -> List a
toListType = foldr Cons Nil

myReverse :: List a -> List a
myReverse xs  = toListType (reverse (toList xs))
